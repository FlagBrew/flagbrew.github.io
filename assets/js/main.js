...
var cacheServer = "https://cachebox.fm1337.com"
...
function getData(repo) {
    ...
    $.getJSON(cacheServer + "/api/repos/" + repo, function (data) {
        ...
        if ($.isArray(data['releases']) && data['releases'].length > 0) {
            // release title
            document.getElementById("latestVersion" + repo).innerHTML = "<li class='list-group-item'>Latest release: " + data['releases'][0].tag_name + " (" + data['releases'][0].published_at.substring(0, 10) + ")</li>";
...
if ($("#projects").length > 0) {
    $("#page-info").append("Check out our projects below or visit us on github!")
    loadProjects()
}
